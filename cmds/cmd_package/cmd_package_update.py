...
def is_user_mange_package(package_install_path, pkg):
    for root, dirs, files in os.walk(package_install_path, topdown=True):
        for name in dirs:
...
def install_git_package(package_install_path, package_name, package_info, package_url, ver_sha, upstream_changed, url_origin):
    try:
        repo_path = os.path.join(package_install_path, package_name)
        ...
        execute_command(clone_cmd, cwd=package_install_path)
    ...
    if os.path.isfile(gitmodules_path):
        ...
        update_submodule(repo_path, package_name == "ESP-IDF")

    return True


def install_not_git_package(package, package_info, package_download_path, package_url, package_install_path, package_name):
    result = True
    # Download a package of compressed package type.
    if not package.download(package_info["ver"], package_download_path, package_url):
        return False

    pkg_dir = package.get_filename(package_info["ver"])
    pkg_dir = os.path.splitext(pkg_dir)[0]
    package_path = os.path.join(package_download_path, package.get_filename(package_info["ver"]))
...
def install_package(package_info, args):
    """Install the required packages."""

    result = True
    package_download_path = args.package_download_path  # os.path.join(env_root, "local_pkgs")
    package_install_path = args.package_install_path  # os.path.join(bsp_root, "packages")
    force_update = args.package_update_force
    pkgs_root = args.package_index_path

    if not force_update:
        logging.info("Begin to check if it's an user managed package {0}, {1} \n".format(package_install_path, package_info))
        if is_user_mange_package(package_install_path, package_info):
    ...
    if is_git_url(package_url):
        if not install_git_package(
            package_install_path, pkgs_name_in_json, package_info, package_url, ver_sha, upstream_changed, url_from_json
        ):
            result = False
    else:
        if not install_not_git_package(
            package, package_info, package_download_path, package_url, package_install_path, pkgs_name_in_json
        ):
            result = False
    return result
...
def update_latest_packages(sys_value, args):
    ...
    package_install_path = sys_value[5]
    ...
    for pkg in read_back_pkgs_json:
        ...
        if pkg["ver"] == "latest_version" or pkg["ver"] == "latest":
            repo_path = os.path.join(package_install_path, pkgs_name_in_json)
            ...
            update_submodule(repo_path, pkgs_name_in_json == "ESP-IDF")
...
def pre_package_update(args):
    ...
    if not os.path.isfile(args.package_install_info):
        ...
        print("No system configuration file : %s." % args.package_install_info)
    ...
    package_install_path = args.package_install_path
    if not os.path.exists(package_install_path):
        os.makedirs(package_install_path, exist_ok=True)
        # os.chdir(package_install_path)
        fp = open(os.path.join(package_install_path, "pkgs.json"), "w")
        fp.write("[]")
        fp.close()

        fp = open(os.path.join(package_install_path, "pkgs_error.json"), "w")
        fp.write("[]")
        fp.close()
        # os.chdir(bsp_root)

    # prepare target packages file
    dbsqlite_pathname = os.path.join(package_install_path, "packages.dbsqlite")
    ...
    newpkgs = kconfig.parse(args.package_install_info)

    # regenerate file : packages/pkgs.json
    package_json_filename = os.path.join(package_install_path, "pkgs.json")
    if not os.path.exists(package_json_filename):
        # os.chdir(package_install_path)
        fp = open(package_json_filename, "w")
        fp.write("[]")
        fp.close()
        # os.chdir(bsp_root)
    ...
    pkgs_error_list_fn = os.path.join(package_install_path, "pkgs_error.json")
    if not os.path.exists(pkgs_error_list_fn):
        # os.chdir(package_install_path)
        fp = open(pkgs_error_list_fn, "w")
        fp.write("[]")
        fp.close()
        # os.chdir(bsp_root)
    ...
    if not os.path.isfile(os.path.join(package_install_path, "SConscript")):
        with open(os.path.join(package_install_path, "SConscript"), "w") as f:
            f.write(str(Bridge_SConscript))

    return [oldpkgs, newpkgs, package_error, package_json_filename, pkgs_error_list_fn, package_install_path, dbsqlite_pathname]
...
def error_packages_handle(error_packages_list, read_back_pkgs_json, package_filename, args):
    ...
    if len(error_packages_list):
        ...
        for package in error_packages_list:
            if install_package(package, args):
                print(
                    "\n==============================> %s %s update done \n"
...
def get_package_remove_path(pkg, package_install_path):
    dir_path = pkg["path"]
    ver = pkg["ver"]
    ...
    remove_path = os.path.join(package_install_path, dir_path)
    remove_path_ver = get_package_folder(remove_path, ver)
    return remove_path_ver


def handle_download_error_packages(sys_value, args):
    ...
    for pkg in package_json:
        remove_path = get_package_remove_path(pkg, package_install_path)
    ...
    get_flag = error_packages_handle(error_packages_list, package_json, package_filename, args)
...
def delete_useless_packages(sys_value):
    ...
    if len(package_delete_error_list):
        for error_package in package_delete_error_list:
            remove_path_with_version = get_package_remove_path(error_package, package_install_path)
...
def is_git_package(pkg, package_install_path):
    remove_path_with_version = get_package_remove_path(pkg, package_install_path)
            ...
            try:
                ...
                logging.warning(
                    ...
                    % (
                        ...
                        git_folder_to_remove.encode("utf-8"),
                        e,
                    )
                )
...
def remove_packages(sys_value, force_update):
    ...
    for pkg in case_delete:
        ...
        if is_git_package(pkg, package_install_path):
...
def install_packages(sys_value, args):
    """
...
def package_update(args):
    ...
    sys_value = pre_package_update(args)
    if not sys_value:
        return
    ...
    if not remove_packages(sys_value, args):
        return

    # 2.in new not in old : Software packages to be installed.
    if not install_packages(sys_value, args):
        flag = False

    # 3.handle download error packages.
    if not handle_download_error_packages(sys_value, args):
        flag = False

    # 4.update the software packages, which the version is 'latest'
    if not update_latest_packages(sys_value, args):
        flag = False
